...
def is_fair_game(*player_ranks):
    ...
    if not player_ranks:
        return False
    elif max(player_ranks)- min(player_ranks) > 5:
        return False
    elif len(player_ranks) % 2 != 0:
        return False
    elif max(player_ranks) - min(player_ranks) <= 5:
...
'game_6': 'not a fair game'}
    ...
    for key, values in games.items():
        if is_fair_game(*values) is True:
            result = []
...
def count_even(nums):
    ...
    0
    """
    return sum([number % 4 == 0 for number in nums])
...
('positional_3', 'G'), ('keyword_0_L', 'O'), ('keyword_1_I', 'S')]
    ...
    """
    return [('positional_' + str(ind), value) for ind, value
            in enumerate(args)] + \
    [(('keyword_' + str(key) + '_' + str(values)), kwargs[values])
     for key, values in enumerate(kwargs)]
...
'you have to be twice as smart as the person who writes the code!'}])
    ...
    2
    """
    return sum([1 for elements in posts for words in hashtags
                if words in elements['hashtags']])
